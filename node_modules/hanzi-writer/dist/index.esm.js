...
function generateStrokes({
  ...
  const isInRadical = strokeNum => (radStrokes?.indexOf(strokeNum) ?? -1) >= 0;
...
class Quiz {
  ...
  endUserStroke() {
    ...
    this._renderState.run(removeUserStroke(this._userStroke.id, this._options.drawingFadeDuration ?? 300)); // skip single-point strokes
  ...
  render(props) {
    ...
    if (colorsChanged || strokes !== this._oldProps?.strokes) {
      for (let i = 0; i < this._strokeRenderers.length; i++) {
        if (!colorsChanged && this._oldProps?.strokes && strokes[i] === this._oldProps.strokes[i]) {
          continue;
        }
...
class StrokeRenderer$1 extends StrokeRendererBase {
  ...
  render(ctx, props) {
    ...
    if (this._path2D) {
      ...
      this._pathCmd?.(ctx); // wechat bugs out if the clip path isn't stroked or filled
...
class HanziWriter {
  ...
  showCharacter(options = {}) {
    ...
    return this._withData(() => this._renderState?.run(showCharacter('main', this._character, typeof options.duration === 'number' ? options.duration : this._options.strokeFadeDuration)).then(res => {
  ...
  hideCharacter(options = {}) {
    ...
    return this._withData(() => this._renderState?.run(hideCharacter('main', this._character, typeof options.duration === 'number' ? options.duration : this._options.strokeFadeDuration)).then(res => {
      options.onComplete?.(res);
      return res;
    }));
  }
  ...
  animateCharacter(options = {}) {
    this.cancelQuiz();
    return this._withData(() => this._renderState?.run(animateCharacter('main', this._character, this._options.strokeFadeDuration, this._options.strokeAnimationSpeed, this._options.delayBetweenStrokes)).then(res => {
      options.onComplete?.(res);
      return res;
    }));
  }
  ...
  animateStroke(strokeNum, options = {}) {
    this.cancelQuiz();
    return this._withData(() => this._renderState?.run(animateSingleStroke('main', this._character, strokeNum, this._options.strokeAnimationSpeed)).then(res => {
      options.onComplete?.(res);
      return res;
    }));
  }
  ...
  showOutline(options = {}) {
    ...
    return this._withData(() => this._renderState?.run(showCharacter('outline', this._character, typeof options.duration === 'number' ? options.duration : this._options.strokeFadeDuration)).then(res => {
      options.onComplete?.(res);
      return res;
    }));
  }
  ...
  hideOutline(options = {}) {
    ...
    return this._withData(() => this._renderState?.run(hideCharacter('outline', this._character, typeof options.duration === 'number' ? options.duration : this._options.strokeFadeDuration)).then(res => {
      options.onComplete?.(res);
      return res;
    }));
  }
  ...
  updateColor(colorName, colorVal, options = {}) {
    ...
    return this._withData(() => this._renderState?.run(mutations).then(res => {
      options.onComplete?.(res);
      return res;
    }));
  }
